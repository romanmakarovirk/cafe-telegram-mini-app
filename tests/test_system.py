...
# Env vars and sys.path setup moved to tests/conftest.py
...
class TestProcessPaidOrder:
    ...
    async def test_process_paid_order_full_cycle(self):
        """Полный цикл: pending → paid → 1c_sync → preparing (YooKassa handles fiscal)."""
        m = self.m
        order_id = self._create_test_order()

        # Mock sync_order_to_1c (no fiscal mock — YooKassa handles fiscalization)
        mock_sync = AsyncMock(return_value=MagicMock(
            success=True, document_id="doc-guid-456", error=""
        ))

        with patch("integrations.accounting.sync_order_to_1c", mock_sync):
    ...
    async def test_process_paid_order_idempotent(self):
        ...
        # sync должен быть вызван ровно 1 раз (second call exits early)
...
class TestApiEndpoints:
    ...
    def test_app_config(self):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["checkout_mode"] == "yookassa"
    ...
    def test_create_payment_unauthorized(self):
        resp = self.client.post("/api/payment/create/1")
        assert resp.status_code == 401
...
class TestConcurrentSafety:
    ...
    async def test_double_process_only_one_succeeds(self):
        ...
        # В идеале только 1 вызов sync (с PostgreSQL будет ровно 1)
...
class TestHealthCheckExtended:
    ...
    def test_healthz_structure(self):
        ...
        assert "yookassa" in data["checks"]
...
class TestFiscalQueueAdvanced(_AppTestBase):
    ...
    def test_fiscal_queue_not_created_on_payment(self):
        """YooKassa handles fiscalization — no FiscalQueue created on payment."""
        order = self._create_order()
        oid = order["order_id"]

        self._confirm_payment(oid)

        # Проверяем что FiscalQueue НЕ создаётся (ЮKassa фискализирует автоматически)
        with self.Session() as s:
            fq = s.scalars(
            ...
            assert fq is None, "FiscalQueue should NOT be created — YooKassa handles fiscal"
...
class TestPaymentHacker(_AppTestBase):
    ...
    def test_create_payment_invalid_order_id_zero(self):
        ...
        resp = self.client.post("/api/payment/create/0?dev_user_id=1")
        assert resp.status_code == 422

    def test_create_payment_invalid_order_id_negative(self):
        """order_id=-1 должен быть отклонён."""
        resp = self.client.post("/api/payment/create/-1?dev_user_id=1")
        assert resp.status_code == 422

    def test_check_status_invalid_order_id(self):
        """order_id=0 для check-status отклоняется."""
        resp = self.client.get("/api/payment/check-status/0?dev_user_id=1")
        assert resp.status_code == 422
    ...
    def test_payment_for_already_paid_order(self):
        ...
        resp = self.client.post(f"/api/payment/create/{oid}?dev_user_id=1")
        body = resp.json()
...
class TestApiHacker(_AppTestBase):
    ...
    def test_order_id_string_rejected(self):
        ...
        resp = self.client.post("/api/payment/create/abc?dev_user_id=1")
        assert resp.status_code == 422
...
class TestRefundNoFiscalQueue:
    """YooKassa handles fiscal receipts — refund should NOT create FiscalQueue."""

    def test_refund_does_not_create_fiscal_queue(self):
        """Verify that the refund handler does NOT create FiscalQueue (YooKassa auto-fiscal)."""
        import inspect
        import bot_handlers

        source = inspect.getsource(bot_handlers.handle_refund)

        # Must use refund_yookassa_payment
        assert "refund_yookassa_payment" in source, (
            "/refund must call refund_yookassa_payment"
        )
        # Must NOT create FiscalQueue (YooKassa handles fiscal automatically)
        assert "FiscalQueue" not in source, (
            "/refund must NOT create FiscalQueue — YooKassa handles fiscal receipts"
        )
...
class TestCallbackWithForUpdate:
    """Webhook handler must use with_for_update() to prevent race conditions."""

    def test_callback_handler_has_for_update(self):
        """Verify that yookassa_webhook route uses with_for_update in its query."""
        import inspect
        from routes import yookassa_webhook

        source = inspect.getsource(yookassa_webhook)
        assert "with_for_update()" in source, (
            "yookassa_webhook must use with_for_update() to prevent concurrent "
            "processing of the same order from duplicate webhooks"
        )
...
class TestE2EFullPaymentCycle(_E2ETestBase):
    """E2E: Полный цикл заказ → оплата → preparing → ready (YooKassa handles fiscal)."""

    @pytest.mark.asyncio
    async def test_payment_to_ready(self):
        """created → _process_paid_order → paid/preparing → ready (no Phase 1/2 fiscal)."""
        m = self.m
        oid = self._create_test_order()

        # Оплата (YooKassa handles fiscal automatically, no fiscalize_order mock needed)
        mock_sync = AsyncMock(return_value=MagicMock(success=False, error="disabled"))

        with patch("integrations.accounting.sync_order_to_1c", mock_sync), \
             patch("services.bot_setup") as mock_bot_setup:
        ...
        # Проверяем: заказ оплачен, preparing
        with self.Session() as session:
            order = session.get(m.Order, oid)
            assert order.payment_status == "paid"
            assert order.status == "preparing"

            # FiscalQueue НЕ должна создаваться (YooKassa)
            fq_count = session.scalars(
                m.select(m.FiscalQueue).where(m.FiscalQueue.order_id == oid)
            ).all()
            assert len(fq_count) == 0, "No FiscalQueue — YooKassa handles fiscal"

        # --- Ready ---
        with patch("bot_handlers.notify_customer", new_callable=AsyncMock):
        ...
        # Проверяем: заказ ready, без Phase 2 fiscal
        with self.Session() as session:
            order = session.get(m.Order, oid)
...
class TestE2ERefundFlow(_E2ETestBase):
    """E2E: Полный цикл возврата — оплата → /refund → YooKassa refund."""

    @pytest.mark.asyncio
    async def test_refund_updates_status(self):
        """paid → /refund → refund_pending → YooKassa ok → refunded (no FiscalQueue)."""
        m = self.m
        oid = self._create_test_order(
            status="preparing",
            payment_status="paid",
            gateway_order_id="yk-pay-12345",
        )

        # Mock YooKassa refund — успешный
        from payments.yookassa_payment import YookassaRefundResult
        mock_refund = AsyncMock(return_value=YookassaRefundResult(
            success=True, refund_id="yk-ref-123", status="succeeded"
        ))

        with patch("payments.yookassa_payment.refund_yookassa_payment", mock_refund), \
        ...
        # Проверяем: refunded + cancelled, NO FiscalQueue (YooKassa auto-fiscal)
        with self.Session() as session:
            order = session.get(m.Order, oid)
            assert order.payment_status == "refunded"
            assert order.status == "cancelled"

            # FiscalQueue НЕ создаётся — YooKassa автоматически фискализирует возврат
            fq_count = session.scalars(
                m.select(m.FiscalQueue).where(m.FiscalQueue.order_id == oid)
            ).all()
            assert len(fq_count) == 0, "No FiscalQueue — YooKassa handles refund fiscal"

        # YooKassa refund вызван с правильными аргументами
        mock_refund.assert_called_once_with("yk-pay-12345", 1200)

    @pytest.mark.asyncio
    async def test_refund_failure_rolls_back(self):
        """YooKassa refund failure → статус возвращается в paid."""
        m = self.m
        oid = self._create_test_order(
            status="preparing",
            payment_status="paid",
            gateway_order_id="yk-pay-fail",
        )
        ...
        with self.Session() as session:
            order = session.get(m.Order, oid)
            assert order.payment_status == "paid", "Должен откатиться при ошибке YooKassa"
...
class TestE2EStoplistRefundFlow(_E2ETestBase):
    """E2E: Оплата → стоп-лист → YooKassa refund (auto-fiscal)."""

    @pytest.mark.asyncio
    async def test_stoplist_refund_cancels_order(self):
        """Стоп-лист при оплате: YooKassa refund → cancelled/refunded (no FiscalQueue)."""
        m = self.m
        oid = self._create_test_order(gateway_order_id="yk-pay-stoplist")
        ...
        from payments.yookassa_payment import YookassaRefundResult
        mock_refund = AsyncMock(return_value=YookassaRefundResult(
            success=True, refund_id="yk-ref-stoplist", status="succeeded"
        ))
        mock_sync = AsyncMock(return_value=MagicMock(success=False, error="disabled"))
        ...
        with self.Session() as session:
            ...
            # НЕТ FiscalQueue — YooKassa автоматически создаёт чеки
            fq_all = session.scalars(
                m.select(m.FiscalQueue).where(m.FiscalQueue.order_id == oid)
            ).all()
            assert len(fq_all) == 0, "No FiscalQueue — YooKassa handles fiscal"

        # YooKassa refund должен быть вызван
        mock_refund.assert_called_once()

    @pytest.mark.asyncio
    async def test_stoplist_refund_failure_alerts_admin(self):
        """Стоп-лист + YooKassa refund failure → refund_failed + alert_admin."""
        m = self.m
        oid = self._create_test_order(gateway_order_id="yk-pay-fail-stop")
...
class TestE2ENoFiscalRetryNeeded(_E2ETestBase):
    """YooKassa handles fiscal — no fiscal retry worker needed for payments."""

    @pytest.mark.asyncio
    async def test_payment_does_not_create_fiscal_queue(self):
        """_process_paid_order does NOT create FiscalQueue (YooKassa auto-fiscal)."""
        m = self.m
        oid = self._create_test_order()
        ...
        with self.Session() as session:
            ...
            # No FiscalQueue created
            fq_count = session.scalars(
                m.select(m.FiscalQueue).where(m.FiscalQueue.order_id == oid)
            ).all()
            assert len(fq_count) == 0, "YooKassa handles fiscal — no FiscalQueue needed"
...
class TestWebhookReplayAttack(_AppTestBase):
    """Один и тот же webhook 3 раза подряд → заказ обработан ровно 1 раз."""

    def test_duplicate_callback_is_idempotent(self):
        """Replay attack: 3 одинаковых webhook → _process_paid_order вызван max 1 раз."""
        order = self._create_order()
        oid = order["order_id"]
        ...
        with self.Session() as session:
            ...
            o.gateway_order_id = "yk-pay-replay-test"
            session.commit()

        webhook_body = {
            "event": "payment.succeeded",
            "object": {
                "id": "yk-pay-replay-test",
                "amount": {"value": f"{order['total'] / 100:.2f}" if order['total'] > 100 else f"{order['total']}.00", "currency": "RUB"},
                "paid": True,
                "status": "succeeded",
            },
        }
        # Recalculate correct amount
        with self.Session() as session:
            o = session.get(self.m.Order, oid)
            amount_rub = o.total_amount
            webhook_body["object"]["amount"]["value"] = f"{amount_rub * 100 / 100:.2f}"

        # Mock _process_paid_order so it actually sets payment_status=paid in DB,
        # enabling the webhook handler's duplicate detection on subsequent calls.
        async def mock_process_side_effect(order_id):
            with self.Session() as sess:
                o = sess.get(self.m.Order, order_id)
                if o:
                    o.payment_status = "paid"
                    o.status = "paid"
                    sess.commit()

        mock_process = AsyncMock(side_effect=mock_process_side_effect)

        with patch("payments.yookassa_payment.is_trusted_ip", return_value=True), \
             patch("routes._process_paid_order", mock_process):

            results = []
            for _ in range(3):
                resp = self.client.post("/api/payment/webhook", json=webhook_body)
...
class TestWebhookUntrustedIP(_AppTestBase):
    """Webhook с недоверенного IP → 403."""

    def test_untrusted_ip_rejected(self):
        """Запрос с недоверенного IP → 403."""
        with patch("payments.yookassa_payment.is_trusted_ip", return_value=False):
            resp = self.client.post(
                "/api/payment/webhook",
                json={"event": "payment.succeeded", "object": {"id": "fake-123"}},
            )
            assert resp.status_code == 403

    def test_trusted_ip_accepted(self):
        """Запрос с доверенного IP → 200 (даже если заказ не найден)."""
        with patch("payments.yookassa_payment.is_trusted_ip", return_value=True):
            resp = self.client.post(
                "/api/payment/webhook",
                json={
                    "event": "payment.succeeded",
                    "object": {"id": "nonexistent-pay", "amount": {"value": "100.00"}, "paid": True},
                },
            )
            assert resp.status_code == 200

    def test_invalid_json_rejected(self):
        """Невалидный JSON → 400 или 422."""
        with patch("payments.yookassa_payment.is_trusted_ip", return_value=True):
            resp = self.client.post(
                "/api/payment/webhook",
                content=b"not json",
...
class TestWebhookAmountMismatch(_AppTestBase):
    """Webhook с неверной суммой → amount_mismatch."""

    def test_amount_mismatch_marks_order(self):
        """YooKassa webhook с другой суммой → amount_mismatch."""
        order = self._create_order()
        oid = order["order_id"]

        with self.Session() as session:
            o = session.get(self.m.Order, oid)
            o.gateway_order_id = "yk-pay-mismatch"
            session.commit()
            total_rub = o.total_amount

        # Подставляем неверную сумму (на 500 руб больше)
        wrong_amount_rub = total_rub + 500

        # Mock API verification — возвращает неверную сумму
        mock_api_result = MagicMock()
        mock_api_result.success = True
        mock_api_result.is_paid = True
        mock_api_result.amount = int(wrong_amount_rub * 100)

        with patch("payments.yookassa_payment.is_trusted_ip", return_value=True), \
             patch("payments.yookassa_payment.check_yookassa_payment",
                   AsyncMock(return_value=mock_api_result)), \
            ...
            resp = self.client.post(
                ...
                json={
                    ...
                    "object": {
                        "id": "yk-pay-mismatch",
                        "amount": {"value": f"{wrong_amount_rub}.00", "currency": "RUB"},
                        "paid": True,
                        "status": "succeeded",
                    },
                },
...
class TestWebhookHappyPath(_AppTestBase):
    """Webhook payment.succeeded → заказ оплачен, _process_paid_order вызван."""

    def test_webhook_succeeds_and_processes_order(self):
        """payment.succeeded → API verified → _process_paid_order вызван."""
        order = self._create_order()
        oid = order["order_id"]

        with self.Session() as session:
            o = session.get(self.m.Order, oid)
            o.gateway_order_id = "yk-pay-happy"
            session.commit()
            total_rub = o.total_amount

        mock_process = AsyncMock()

        # Mock API verification — платёж подтверждён, сумма совпадает
        mock_api_result = MagicMock()
        mock_api_result.success = True
        mock_api_result.is_paid = True
        mock_api_result.amount = int(total_rub * 100)
    ...
    def test_webhook_canceled_updates_order_status(self):
        """payment.canceled → заказ становится cancelled."""
        order = self._create_order()
        oid = order["order_id"]

        with self.Session() as session:
            o = session.get(self.m.Order, oid)
            o.gateway_order_id = "yk-pay-cancel"
            session.commit()
...
class TestPaymentWithoutFiscal(_E2ETestBase):
    """YooKassa handles fiscal — _process_paid_order works without fiscal calls."""

    @pytest.mark.asyncio
    async def test_1c_timeout_does_not_block_order(self):
        """1С TimeoutError → заказ preparing, no FiscalQueue."""
        m = self.m
        oid = self._create_test_order()

        mock_sync = AsyncMock(side_effect=asyncio.TimeoutError("1C timeout"))
    ...
    @pytest.mark.asyncio
    async def test_1c_connection_error_does_not_block(self):
        """ConnectionError on 1C → order still moves to preparing."""
        m = self.m
        oid = self._create_test_order()

        mock_sync = AsyncMock(side_effect=ConnectionError("1C unreachable"))
...
class TestCreatePaymentTimeout(_AppTestBase):
    """Таймаут YooKassa create_payment → gateway_order_id сброшен."""
    ...
    def test_create_payment_timeout_resets_marker(self):
        ...
        from payments.yookassa_payment import YookassaPaymentResult
        with patch("payments.yookassa_payment.create_yookassa_payment", new_callable=AsyncMock) as mock_create:
            mock_create.return_value = YookassaPaymentResult(
...
class TestConcurrentProcessPaidOrder(_E2ETestBase):
    ...
    async def test_concurrent_calls_process_once(self):
        """2 asyncio.Task на один заказ → sync вызван max 1 раз."""
        m = self.m
        oid = self._create_test_order()

        call_count = {"sync": 0}

        async def mock_sync_fn(**kwargs):
            call_count["sync"] += 1
            await asyncio.sleep(0.05)
            return MagicMock(success=False, error="disabled")

        with patch("integrations.accounting.sync_order_to_1c", side_effect=mock_sync_fn), \
             patch("services.bot_setup") as mock_bot_setup:
            mock_bot_setup.bot = None
            mock_bot_setup.ADMIN_CHAT_ID = None
            from services import _process_paid_order
        ...
        assert call_count["sync"] == 1, \
            f"Expected 1 sync call, got {call_count['sync']}"
...
class TestAccountingSyncPayloadValidation(_E2ETestBase):
    """1C sync payload содержит все обязательные поля (fiscal handled by YooKassa)."""

    @pytest.mark.asyncio
    async def test_sync_payload_has_required_fields(self):
        """sync_order_to_1c получает items с name, quantity, price, total."""
        m = self.m
        oid = self._create_test_order()
        ...
        async def capture_sync(**kwargs):
            ...
            return MagicMock(success=True, document_id="doc-123", error="")

        with patch("integrations.accounting.sync_order_to_1c", side_effect=capture_sync), \
             patch("services.bot_setup") as mock_bot_setup:
            mock_bot_setup.bot = None
            mock_bot_setup.ADMIN_CHAT_ID = None
            from services import _process_paid_order
        ...
        computed = sum(i["price"] * i["quantity"] for i in captured["items"])
        assert captured["total_amount"] == computed

    @pytest.mark.asyncio
    async def test_no_fiscal_queue_created_on_payment(self):
        """YooKassa handles fiscal — no FiscalQueue on payment."""
        m = self.m
        oid = self._create_test_order()
    ...
    @pytest.mark.asyncio
    async def test_order_moves_to_preparing(self):
        """After _process_paid_order, order is in paid/preparing state."""
        m = self.m
        oid = self._create_test_order()
...
class TestReadyTransitionNoFiscal(_E2ETestBase):
    """preparing → ready: ЮKassa handles fiscal, no FiscalQueue created."""

    @pytest.mark.asyncio
    async def test_ready_transition_no_fiscal_queue(self):
        """preparing → ready: заказ переходит в ready, FiscalQueue НЕ создаётся (ЮKassa)."""
        m = self.m
        oid = self._create_test_order(
        ...
        with self.Session() as session:
            ...
            # No FiscalQueue created — YooKassa handles fiscalization
            fq = session.scalars(
                m.select(m.FiscalQueue).where(
                    m.FiscalQueue.order_id == oid,
                )
            ).first()
            assert fq is None, "YooKassa handles fiscal — no FiscalQueue should be created"
...
class TestCallbackUnknownOrder(_AppTestBase):
    """Webhook на несуществующий payment_id → 200 ok без обработки."""

    def test_callback_unknown_gateway_id(self):
        """payment_id не найден в БД → 200 ok, _process_paid_order не вызван."""
        with patch("payments.yookassa_payment.is_trusted_ip", return_value=True), \
             patch("routes._process_paid_order", new_callable=AsyncMock) as mock_process:
            resp = self.client.post(
                "/api/payment/webhook",
                json={
                    "event": "payment.succeeded",
...
class TestCallbackNonDeposited(_AppTestBase):
    """Webhook с event != 'payment.succeeded' → не обрабатывается."""

    def test_callback_refunded_operation_ignored(self):
        """event='refund.succeeded' → 200 ok, без обработки заказа."""
        with patch("payments.yookassa_payment.is_trusted_ip", return_value=True), \
             patch("routes._process_paid_order", new_callable=AsyncMock) as mock_process:
            resp = self.client.post(
                "/api/payment/webhook",
    ...
    def test_callback_status_zero_ignored(self):
        """event='payment.canceled' → не обрабатывается как оплата."""
        with patch("payments.yookassa_payment.is_trusted_ip", return_value=True), \
             patch("routes._process_paid_order", new_callable=AsyncMock) as mock_process:
            resp = self.client.post(
                "/api/payment/webhook",
...
class TestDoubleRefundRejected(_E2ETestBase):
    ...
    async def test_second_refund_rejected(self):
        ...
        oid = self._create_test_order(
            status="preparing", payment_status="paid",
            gateway_order_id="yk-double-refund",
        )
...
class TestReadyTransitionNoFiscalOperations(_E2ETestBase):
    """preparing → ready: ЮKassa handles fiscal, no fiscal operations needed."""

    @pytest.mark.asyncio
    async def test_ready_no_fiscal_operations(self):
        """preparing → ready: no FiscalQueue, no fiscal calls (ЮKassa handles 54-ФЗ)."""
        m = self.m
        oid = self._create_test_order(
            status="preparing", payment_status="paid",
        )
        ...
        with self.Session() as session:
            ...
            # No FiscalQueue of any type — YooKassa handles all fiscalization
            fq_any = session.scalars(
                m.select(m.FiscalQueue).where(m.FiscalQueue.order_id == oid)
            ).first()
            assert fq_any is None, "YooKassa handles fiscal — no FiscalQueue should be created"
...
class TestExpiredOrderResurrection(_E2ETestBase):
    ...
    async def test_callback_resurrects_expired_order(self):
        """ЮKassa webhook payment.succeeded для expired заказа → _process_paid_order вызван."""
        m = self.m
        oid = self._create_test_order(
            status="cancelled", payment_status="expired",
            gateway_order_id="yk-expired-1",
        )
        ...
        # Expired order has payment_status="expired" which is not in
        # (PAID, AMOUNT_MISMATCH) so webhook should process it
        with self.Session() as session:
            o = session.get(m.Order, oid)
            amount_rub = o.total_amount
        ...
        # Mock API verification — платёж подтверждён
        mock_api_result = MagicMock()
        mock_api_result.success = True
        mock_api_result.is_paid = True
        mock_api_result.amount = int(amount_rub * 100)
        ...
        with patch("payments.yookassa_payment.is_trusted_ip", return_value=True), \
             ...
             patch("routes._process_paid_order", mock_process), \
             patch("bot_handlers.alert_admin", AsyncMock()):
            resp = client.post("/api/payment/webhook", json=webhook_body)
            assert resp.status_code == 200
...
class TestSecurityHeaders(_AppTestBase):
    """Middleware добавляет security headers на все ответы."""

    def test_csp_header_present(self):
        """Content-Security-Policy header присутствует."""
        resp = self.client.get("/healthz")
    ...
    def test_request_id_header(self):
        """X-Request-Id header присутствует (UUID формат)."""
        resp = self.client.get("/healthz")
        request_id = resp.headers.get("x-request-id", "")
        assert len(request_id) == 36, f"Expected UUID format, got: {request_id}"
